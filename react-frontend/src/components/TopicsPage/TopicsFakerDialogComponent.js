...
import topicsFakerFactory from "./topicsFakerFactory";
...
const TopicsFakerDialogComponent = (props) => {
    ...
    const onRun = async () => {
        let fakeData = topicsFakerFactory(fakerCount);

        setLoading(true);
        const promises = fakeData.map((elem) => client.service("topics").create(elem));
...
export default connect(null, mapDispatch)(TopicsFakerDialogComponent);
