...
const CoursesFakerDialogComponent = (props) => {
    ...
    useEffect(() => {
        ...
        client
            .service("topics")
            .find({ query: { $select: ["_id","topicstitle"] } })
            .then((data) => {
              settopicsIds(data.data);
            });

    }, [props.show]);

    const onRun = async () => {
        let fakeData = coursesFakerFactory(fakerCount,topicsIds);
...
export default connect(null, mapDispatch)(CoursesFakerDialogComponent);
